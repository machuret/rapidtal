/**
 * Seed script for Rapid Tile Portal
 * Run AFTER migrations:  npx ts-node --project tsconfig.json db/seed.ts
 *
...
const USERS = [
  { email: "admin@rapidtile.local", password: "admin", role: "super_admin", full_name: "Super Admin", clientSlug: null },
  { email: "client@rapidtile.local", password: "client", role: "client_admin", full_name: "Client Admin", clientSlug: DEMO_CLIENT_SLUG },
  { email: "va@rapidtile.local", password: "va", role: "va", full_name: "Demo VA", clientSlug: DEMO_CLIENT_SLUG },
];
...
async function main() {
  ...
  if (existingClient) {
    ...
    const { data: newClient, error } = await supabase
      .from("clients")
      .insert({ name: DEMO_CLIENT_NAME, slug: DEMO_CLIENT_SLUG })
      .select()
      .single();
  ...
  for (const u of USERS) {
    ...
    if (existing) {
      ...
      if (authError) throw new Error(`Failed to create auth user ${u.email}: ` + authError.message);
  ...
  // 4. Seed a Vault item
  const { error: vaultError } = await supabase.from("vault_items").upsert(
    {
      client_id: clientId,
      source_type: "text",
...
Contact us at hello@democlient.com.au or +61 2 9000 0000.`,
  ...
  if (vaultError) throw new Error("Failed to seed vault: " + vaultError.message);
