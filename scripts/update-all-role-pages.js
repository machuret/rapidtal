...
const roles = [
  // SEO Roles
  'wordpress-seo-specialist', 'shopify-seo-specialist', 'link-building-specialist',
  'technical-seo-specialist', 'local-seo-specialist', 'ecommerce-seo-specialist',
  'seo-content-writer', 'on-page-seo-specialist', 'youtube-seo-specialist',
  'seo-analyst', 'woocommerce-seo-specialist',
  
  // Paid Ads Roles
  'google-ads-specialist', 'facebook-ads-specialist', 'tiktok-ads-specialist',
  'google-shopping-specialist', 'programmatic-advertising-specialist', 'linkedin-ads-specialist',
  'ppc-manager', 'paid-social-specialist', 'youtube-ads-specialist', 'retargeting-specialist',
  
  // Sales Roles
  'sales-development-rep', 'business-development-manager', 'cold-calling-specialist',
  'appointment-setter', 'account-executive', 'account-manager', 'crm-manager',
  'sales-operations-specialist', 'inside-sales-representative', 'customer-success-manager',
  
  // Marketing & Creative Roles
  'copywriter', 'social-media-manager', 'email-marketing-specialist', 'video-editor',
  'graphic-designer', 'content-strategist', 'ugc-manager', 'brand-designer',
  'podcast-producer', 'marketing-copywriter',
  
  // Operations & Admin Roles
  'executive-assistant', 'virtual-assistant', 'marketing-assistant', 'project-manager',
  'data-entry-specialist', 'lead-generation-specialist', 'research-analyst',
  'customer-support-specialist', 'hubspot-administrator', 'operations-manager'
];
...
function slugToComponentName(slug) {
  return 'Hire' + slug.split('-').map(word => 
...
roles.forEach(roleSlug => {
  const dirPath = path.join(__dirname, '../app', `hire-${roleSlug}`);
  const filePath = path.join(dirPath, 'page.tsx');
  
  if (fs.existsSync(filePath)) {
    const componentName = slugToComponentName(roleSlug);
    fs.writeFileSync(filePath, pageTemplate(roleSlug, componentName), 'utf8');
    updatedCount++;
    console.log(`✅ Updated: hire-${roleSlug}/page.tsx`);
  } else {
    console.log(`⚠️  Skipped: hire-${roleSlug}/page.tsx (not found)`);
  }
});

console.log(`\n🎉 Successfully updated ${updatedCount} role pages with breadcrumbs and related roles!`);
